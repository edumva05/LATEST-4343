...
class ButtonsView: UIView {
    ...
    func animageMovingView(sender : UIButton){
        UIView.animate(withDuration: 0.20, delay: 0, options: [UIViewAnimationOptions.curveEaseInOut], animations: {
            ...
            () -> Void in
            self.movingView?.frame = CGRect(x: sender.frame.origin.x + (self.frame.origin.x * 0.8), y: (self.movingView?.frame.origin.y)! , width: sender.frame.size.width * 0.5, height: 2)
